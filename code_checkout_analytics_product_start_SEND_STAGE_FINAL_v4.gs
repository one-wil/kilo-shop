...
function recordPurchaseForSavedOrderUnlocked_(data, orderId, requestId, storeKey) {
  try {
    const session = String(data.checkoutSessionId || data.sessionId || '').trim();
    if (!session) {
      console.warn('⚠️ Purchase Analytics non enregistrée: checkoutSessionId absent', orderId);
...
function buildCheckoutAnalyticsResult_(events, storeKey) {
  ...
  return {
    ...
    summary:{visitors:visitors,sessions:sessionCount,checkoutSessions:begin,storeHomeViews:home,stage1:product,stage2:begin,stage3:wilaya,stage4:delivery,stage5:details,submitAttempts:attempts,sendStage:sendStage,purchases:purchases,conversionRate:pct(purchases,sessionCount),abandoned:abandoned,abandonedRate:pct(abandoned,sessionCount),explicitAbandoned:explicitAbandoned},
    funnel:[
