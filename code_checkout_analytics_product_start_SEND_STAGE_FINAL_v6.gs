...
function doPost(e) {
  ...
  try {
    ...
    // IMPORTANT: once the order is actually saved, register the purchase
    // server-side using the same browser Checkout session. This makes the
    // funnel authoritative even if the browser's Analytics request is lost
    // during redirect/pagehide/network delay. The current ScriptLock is
    // already held, so this helper must NOT acquire the lock again.
    recordPurchaseForSavedOrderUnlocked_(orderData, orderId, requestId, storeKey);
...
// قراءة الطلبات فقط: لا تعيد بناء/تنسيق مخطط ورقة الطلبات في كل طلب Analytics.
function getOrdersSheetReadOnly_() {
  const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  let sheet = ss.getSheetByName(SHEET_NAME);
  if (!sheet) {
    const wanted = String(SHEET_NAME).trim().toLowerCase();
    const sheets = ss.getSheets();
    for (let i = 0; i < sheets.length; i++) {
      if (String(sheets[i].getName()).trim().toLowerCase() === wanted) { sheet = sheets[i]; break; }
    }
  }
  return sheet || null;
}
...
function processOrderDirect(data) {
  ...
  try {
    ...
    // Same server-side purchase reconciliation for the direct admin/API path.
    // The lock is already held by processOrderDirect().
    recordPurchaseForSavedOrderUnlocked_(data, orderId, requestId, storeKey);
...
function apiListOrders(body) {
  ...
  const sheet = getOrdersSheetReadOnly_();
  if (!sheet) return {success:true,orders:[],total:0,page,pageSize,totalPages:1,stats:buildOrderStats([])};
  const idx = getHeaderIndexes(sheet);
  const lastRow = sheet.getLastRow();
  const orders = [];
  ...
  if (lastRow >= DATA_START_ROW) {
    const values = sheet.getRange(
      ...
      Math.max(SHEET_WIDTH, sheet.getLastColumn())
    ).getValues();
...
// Register a purchase after the order has been successfully written to the
// Orders sheet. The caller already owns ScriptLock, so this function never
// waits for or releases the lock.
function recordPurchaseForSavedOrderUnlocked_(data, orderId, requestId, storeKey) {
  try {
    let session = String(data.checkoutSessionId || data.sessionId || '').trim();
    // إذا لم يصل معرف الجلسة من المتصفح، لا نسقط تسجيل purchase بعد نجاح الطلب.
    // نستخدم جلسة احتياطية مرتبطة بالطلب نفسه، مع إبقاء الربط الأصلي إذا وصل.
    if (!session) {
      session = 'ORDER-' + String(orderId || requestId || Date.now()).trim();
      console.warn('⚠️ checkoutSessionId absent; fallback analytics session used:', session);
    }

    const sh = getCheckoutAnalyticsSheet_();
    const last = sh.getLastRow();
    if (last >= 2) {
      const rows = sh.getRange(2, 1, last - 1, Math.max(sh.getLastColumn(), CHECKOUT_ANALYTICS_HEADERS.length)).getValues();
      for (let i = 0; i < rows.length; i++) {
        if (normalizeStoreKey(rows[i][0]) === storeKey &&
            String(rows[i][1] || '').trim() === session &&
            String(rows[i][2] || '').trim() === 'purchase') {
          return {success:true,deduplicated:true};
        }
      }
    }

    const items = sanitizeCheckoutItems_(data.items);
    ...
    row[4] = 6;
    row[5] = String(data.wilaya || '');
    row[6] = String(data.deliveryType || '');
    row[7] = 'server_order';
    row[8] = items.reduce((n, it) => n + (Number(it.quantity) || 1), 0);
    row[9] = Number(data.total || 0);
    row[10] = Number(data.total || 0);
    row[11] = 'server_order:' + String(orderId || '') + '|request:' + String(requestId || '');
    const itemsIndex = CHECKOUT_ANALYTICS_HEADERS.indexOf('Items');
    if (itemsIndex >= 0) row[itemsIndex] = itemJson;
    sh.appendRow(row);
    console.log('✅ Purchase Analytics reconcilié côté serveur:', orderId, session);
    return {success:true};
  } catch (err) {
    // Never reject a successfully saved order because Analytics failed.
    console.error('⚠️ Erreur Purchase Analytics serveur:', err);
...
function recordCheckoutEvent(data) {
  ...
  try {
    ...
    // A purchase can be sent by both the browser and the server-side
    // reconciliation. Keep one purchase event per Checkout session.
    if (event === 'purchase') {
      const last = sh.getLastRow();
      if (last >= 2) {
        const rows = sh.getRange(2, 1, last - 1, Math.max(sh.getLastColumn(), CHECKOUT_ANALYTICS_HEADERS.length)).getValues();
        for (let i = 0; i < rows.length; i++) {
          if (normalizeStoreKey(rows[i][0]) === storeKey &&
              String(rows[i][1] || '').trim() === session &&
              String(rows[i][2] || '').trim() === 'purchase') {
            return {success:true,event:'purchase',sessionId:session,deduplicated:true};
          }
        }
      }
    }
...
function buildCheckoutAnalyticsResult_(events, storeKey) {
  const stageForEvent_ = (event, fallback) => {
    const map = {
      ...
      delivery_selected: 4,
      details_started: 5,
      checkout_submit_attempt: 6,
      purchase: 6,
      checkout_abandoned: Number(fallback || 1)
    };
  ...
  const productIds=ids.filter(id=>sessions[id].product);
  const checkoutIds=ids.filter(id=>sessions[id].begin);
  const uniqueCount=(event, pool=productIds)=>pool.filter(id=>sessions[id].events.some(e=>e.event===event)).length;
  const visitors=ids.length;
  const home=uniqueCount('store_home_view',ids);
  const product=productIds.length;
  const begin=uniqueCount('begin_checkout');
  ...
  // جلسة Checkout تبدأ من عرض تفاصيل المنتج، وليس من الصفحة الرئيسية.
  // أي جلسة شاهدت تفاصيل منتج ولم تُتم الشراء تُعتبر غير مكتملة، حتى لو غادرت قبل بدء Checkout.
  const sessionIds=productIds;
  const sessionCount=sessionIds.length;
  const purchases=sessionIds.filter(id=>sessions[id].purchase).length;
  // المرحلة 6 تجمع محاولات الإرسال وإتمام الطلب على مستوى الجلسة الفريدة.
  // الجلسة التي تحتوي الحدثين معًا تُحسب مرة واحدة فقط.
  const sendStage=sessionIds.filter(id=>sessions[id].events.some(e=>e.event==='checkout_submit_attempt'||e.event==='purchase')).length;
  const abandoned=sessionIds.filter(id=>!sessions[id].purchase).length;
  const explicitAbandoned=sessionIds.filter(id=>sessions[id].abandoned&&!sessions[id].purchase).length;
  ...
  const abandonedSessions=productIds.filter(id=>!sessions[id].purchase).map(id=>{
    const s=sessions[id]; const last=s.events[s.events.length-1]||{};
    return {sessionId:id,lastStage:s.lastStage||last.stage||1,wilaya:s.wilaya||'',deliveryType:s.deliveryType||'',lastEvent:last.event||'',lastAt:last.timestamp||'',cartCount:last.cartCount||0,cartValue:last.cartValue||0,total:last.total||s.total||0,items:s.items||last.items||[]};
  ...
  return {
    ...
    summary:{visitors:visitors,sessions:sessionCount,checkoutSessions:sessionCount,storeHomeViews:home,stage1:product,stage2:begin,stage3:wilaya,stage4:delivery,stage5:details,submitAttempts:attempts,sendStage:sendStage,purchases:purchases,conversionRate:pct(purchases,sessionCount),abandoned:abandoned,abandonedRate:pct(abandoned,sessionCount),explicitAbandoned:explicitAbandoned},
    funnel:[
      {stage:1,label:'عرض تفاصيل المنتج',sessions:product,rate:pct(product,sessionCount)},
      {stage:2,label:'السلة / بدء Checkout',sessions:begin,rate:pct(begin,sessionCount)},
      {stage:3,label:'اختيار الولاية',sessions:wilaya,rate:pct(wilaya,sessionCount)},
      {stage:4,label:'طريقة التوصيل',sessions:delivery,rate:pct(delivery,sessionCount)},
      {stage:5,label:'بيانات العميل',sessions:details,rate:pct(details,sessionCount)},
      {stage:6,label:'إرسال الطلب',sessions:sendStage,rate:pct(sendStage,sessionCount)}
    ],
