...
function buildCheckoutAnalyticsResult_(events, storeKey) {
  ...
  // الجلسة تبدأ من زيارة الصفحة الرئيسية، وليس من بدء Checkout.
  // أي جلسة لم تُتم الشراء تُعتبر جلسة غير مكتملة، حتى لو غادرت من المرحلة 1 أو 2.
  const sessionIds=ids.filter(id=>sessions[id].home);
  ...
  const abandonedSessions=sessionIds.filter(id=>!sessions[id].purchase).map(id=>{
