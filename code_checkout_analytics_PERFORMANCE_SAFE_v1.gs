...
function ensureOrderSheetSchema(sheet) {
  ...
  // Performance-safe fast path: the schema is already correct.
  // Do not reformat/rebuild validation/title/backfill on every API request.
  if (row2.slice(0, SHEET_WIDTH).join('|') === SHEET_HEADERS.join('|')) return;
