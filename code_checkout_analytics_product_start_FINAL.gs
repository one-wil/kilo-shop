...
function buildCheckoutAnalyticsResult_(events, storeKey) {
  ...
  // Checkout funnel starts at product detail view, not homepage.
  ...
  const purchases=productIds.filter(id=>sessions[id].purchase).length;
  const abandoned=productIds.filter(id=>!sessions[id].purchase).length;
  const explicitAbandoned=productIds.filter(id=>sessions[id].abandoned&&!sessions[id].purchase).length;
  ...
  return {
    ...
    summary:{visitors:visitors,storeHomeViews:home,productViews:product,checkoutSessions:begin,stage1:product,stage2:begin,stage3:wilaya,stage4:delivery,stage5:details,submitAttempts:attempts,purchases:purchases,conversionRate:pct(purchases,product),abandoned:abandoned,abandonedRate:pct(abandoned,product),explicitAbandoned:explicitAbandoned},
    funnel:[
      {stage:1,label:'عرض تفاصيل المنتج',sessions:product,rate:pct(product,product)},
      {stage:2,label:'السلة / بدء Checkout',sessions:begin,rate:pct(begin,product)},
      {stage:3,label:'اختيار الولاية',sessions:wilaya,rate:pct(wilaya,product)},
      {stage:4,label:'طريقة التوصيل',sessions:delivery,rate:pct(delivery,product)},
      {stage:5,label:'بيانات العميل',sessions:details,rate:pct(details,product)},
      {stage:6,label:'إرسال الطلب',sessions:attempts,rate:pct(attempts,product)},
      {stage:7,label:'إتمام الطلب',sessions:purchases,rate:pct(purchases,product)}
    ],
